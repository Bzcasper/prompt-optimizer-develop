...
// 类型定义
export type TemplateType = 'optimize' | 'iterate' | 'user-optimize' | 'content-creation' | 'video-creation' | 'chained';
  ...
  public loadTemplates(): StaticTemplateCollection {
    ...
    try {
      ...
      console.log(`✅ 成功加载 ${Object.keys(all).length} 个模板`, {
        ...
        'chained': Object.keys(byType['chained'].zh).length + Object.keys(byType['chained'].en).length
      });
