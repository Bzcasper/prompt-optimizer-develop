...
import { registerBuiltInTools, getMCPToolDefinitions, executeTool } from './adapters/tool-registration.js';
import {
...
async function createServerInstance(config: any) {
  ...
  const { registry: agentRegistry, llmConfigManager, executionManager } = await registerBuiltInAgents(
    storageProvider,
    modelManager,
    llmService
  );

  return { server, coreServices, toolRegistry, agentRegistry, llmConfigManager, executionManager };
}
...
async function setupServerHandlers(
  ...
  const [userOptimizeOptions, systemOptimizeOptions, iterateOptions, userDefaultId, systemDefaultId, iterateDefaultId, contentTemplates] = await Promise.all([
    ...
    const promptOptimizationTools = [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            template: {
              type: "string",
              description: `选择优化模板，不同模板适用于不同场景：\n${userOptimizeOptions.map((opt: any) => `- ${opt.label}：${opt.description}`).join('\n')}`,
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            template: {
              type: "string",
              description: `选择优化模板，不同模板适用于不同场景：\n${systemOptimizeOptions.map((opt: any) => `- ${opt.label}：${opt.description}`).join('\n')}`,
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            template: {
              type: "string",
              description: `选择迭代优化模板，不同模板有不同的改进策略：\n${iterateOptions.map((opt: any) => `- ${opt.label}：${opt.description}`).join('\n')}`,
      ...
      },
      {
        ...
        description: "使用专业模板生成高质量内容。支持文章写作、营销文案、技术文档等多种内容类型。\n\n主要功能：\n- 使用预定义的专业模板\n- 支持变量替换和自定义参数\n- 生成结构化、专业的内容\n- 适用于多种内容创作场景\n\n支持的内容类型：\n- 文章写作 (Article Writer)\n- 营销文案 (Marketing Copy)\n- 技术文档 (Technical Documentation)\n- 以及更多专业模板\n\n使用场景示例：\n- 写SEO优化的博客文章\n- 创建产品营销文案\n- 生成技术文档和教程\n- 撰写专业报告和分析",
        inputSchema: {
          type: "object",
          properties: {
            templateId: {
              type: "string",
              description: `选择内容生成模板：\n${contentTemplates.map((template: any) => `- ${template.id}：${template.description}`).join('\n')}`,
              enum: contentTemplates.map((template: any) => template.id)
            },
            variables: {
              type: "object",
              description: "模板变量，根据选择的模板提供相应的变量值。例如：{topic: 'AI技术', audience: '开发者', wordCount: 1500}"
            },
            systemPrompt: {
              type: "string",
      ...
      },
      {
        ...
        description: "迭代式内容生成和优化。通过多轮迭代改进生成的内容质量。\n\n主要功能：\n- 首先生成初始内容\n- 根据反馈进行多轮优化\n- 支持自定义优化要求\n- 逐步提升内容质量\n\n工作流程：\n1. 使用基础模板生成初始内容\n2. 根据具体要求进行迭代优化\n3. 每次迭代都基于前一版本改进\n4. 直到达到满意的质量标准\n\n适用场景：\n- 需要高质量内容的创作任务\n- 内容需要经过多次修改和完善\n- 对输出质量有较高要求的项目\n- 需要专业化或定制化内容的生成",
        inputSchema: {
          type: "object",
          properties: {
            templateId: {
            ...
            refinementPrompt: {
              type: "string",
              description: "内容优化要求，描述如何改进生成的内容。例如：'使语言更专业' 或 '增加更多技术细节'"
            },
            maxIterations: {
              type: "number",
    ...
    try {
      switch (name) {
        case "optimize-user-prompt": {
          ...
          if (!prompt) {
            throw MCPErrorHandler.createValidationError("缺少必需参数 'prompt'");
          }
          ...
          if (!mcpModel || !mcpModel.enabled) {
            throw MCPErrorHandler.createModelConfigurationError("MCP 默认模型未配置或未启用，请检查环境变量配置");
          }
        ...
        case "iterate-prompt": {
          ...
          if (!requirements) {
            throw MCPErrorHandler.createValidationError("缺少必需参数 'requirements'");
          }
        ...
        case "generate-content": {
          ...
          if (!templateId) {
            throw MCPErrorHandler.createValidationError("缺少必需参数 'templateId'");
          }

          if (!variables || typeof variables !== 'object') {
            throw MCPErrorHandler.createValidationError("缺少必需参数 'variables' 或变量格式无效");
          }
        ...
        case "generate-content-iterative": {
          const { templateId, variables, refinementPrompt, maxIterations, systemPrompt } = args as {
            templateId?: string;
            variables?: Record<string, any>;
          ...
          if (!refinementPrompt) {
            throw MCPErrorHandler.createValidationError("缺少必需参数 'refinementPrompt'");
          }
        ...
        default:
          ...
          if (toolRegistry.getTool(name)) {
            ...
            return await executeTool(toolRegistry, name, args as Record<string, any>, sessionId, 'mcp-server');
          }
          ...
          if (agentRegistry.getAgent(name)) {
            ...
            if (!task) {
              throw MCPErrorHandler.createValidationError("缺少必需参数 'task'");
            }
            
            return await executeAgent(executionManager, name, task, parameters || {}, sessionId, userId, options);
          }
...
async function main() {
  ...
  try {
    ...
    // 创建服务器实例并设置处理器
    const { server, toolRegistry, agentRegistry } = await createServerInstance(config);
    await setupServerHandlers(server, coreServices, toolRegistry, agentRegistry);
