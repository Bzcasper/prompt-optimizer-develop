...
export class CoreServicesManager {
  ...
  private async validateServices(): Promise<void> {
    const services = [
      ...
      { name: 'ContentGenerationService', service: this.contentGenerationService }
    ];
  ...
  getContentGenerationService(): IContentGenerationService {
    if (!this.initialized || !this.contentGenerationService) {
      throw new Error('CoreServicesManager not initialized or ContentGenerationService not available');
    }
